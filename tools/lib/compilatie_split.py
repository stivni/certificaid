"""Library voor het splitsen van een KB-compilatie-MD in losse KB-bodies.
...
KB-grenzen worden gedetecteerd via FOD-page-headers van de vorm
``"FOD Financiën … Btw KB nr. X"`` (waar X een nummer of een datum kan zijn).
Aaneengesloten markers met dezelfde KB-id worden tot één range gevoegd; per
KB kunnen meerdere niet-aaneengesloten ranges voorkomen.
"""
...
# FOD page-header regex — vangt zowel "Btw KB nr. 1" als "Btw KB 07.06.2007".
_FOD_HEADER = re.compile(
...
@dataclass
class SplitConfig:
    """Beschrijft één gewenste KB-split.

    Attributes:
        kb_id: KB-identifier zoals die in de FOD-headers voorkomt
            (bv. ``"1"``, ``"2bis"``, ``"07.06.2007"``).
...
def detect_kb_boundaries(text: str) -> list[tuple[str, int, int]]:
    """Detecteer KB-grenzen in een compilatie-tekst.

    Geeft een lijst ``[(kb_id, start_line, end_line), ...]`` terug — niet-
    overlappend, in volgorde van voorkomen. Aaneengesloten FOD-headers met
    dezelfde KB-id vormen één range; zodra een marker met een andere KB-id
    ...
    for i, line in enumerate(lines):
        m = _FOD_HEADER.match(line.strip())
        if m:
...
def split_btw_compilatie(
    ...
    """Splits compilatie-tekst in losse KB-bodies volgens splits_config.
    ...
    Een KB-id die niet in de tekst voorkomt levert een lege body op
