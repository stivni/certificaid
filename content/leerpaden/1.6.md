...
- ISA's (International Standards on Auditing) — IAASB
...
|---|---|
| `Themafiche controleopdracht` *(nog te maken)* | **Kern** — de zes fases van een audit + ISA-mapping per fase |
| `Themafiche controleverklaring` *(nog te maken)* | **Kern** — vier oordeel-types als beslisboom (materieel × diepgaand) |
| `Themafiche bijzondere mandaten` *(nog te maken)* | **Kern** — overzicht van alle WVV-verslagen + per-verrichting checklist |
| `Themafiche onafhankelijkheid` *(nog te maken)* | **Vereist** — IESBA-bedreigingen + Belgische incompatibiliteiten |
