...
2. **Eenrichtings-koppeling: programmaonderdeel-JSON kent concepten, concept-records weten van niets**:
   ...
   ```json
   // data/programmaonderdelen/<onderdeel>.json — ENIGE WAARHEID
   "kenniselementen": [
...
- Programmaonderdeel-bestanden leven in `data/programmaonderdelen/` (één JSON per programmaonderdeel), versie-getagd. Schema breidt uit met `concepten`-lijst per kenniselement (en optioneel per taak/doelstelling).
