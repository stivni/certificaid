...
1. **Het examenprogramma wordt expliciet ingelezen** als gestructureerde data — één bestand per programmaonderdeel in `data/programma/programmaonderdelen/`, met taken, doelstellingen en kenniselementen elk gecodeerd. Provenance-getagd zoals elk ander artefact (ADR-004).

2. **Eenrichtings-koppeling: programmaonderdeel-JSON kent concepten, concept-records weten van niets**:

   ```json
   // data/programma/programmaonderdelen/<onderdeel>.json — ENIGE WAARHEID
   "kenniselementen": [
...
- Programmaonderdeel-bestanden leven in `data/programma/programmaonderdelen/` (één JSON per programmaonderdeel), versie-getagd. Schema breidt uit met `concepten`-lijst per kenniselement (en optioneel per taak/doelstelling).
