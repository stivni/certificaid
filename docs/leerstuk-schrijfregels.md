...
11. **Introduceer een term vóór je hem gebruikt** — voluit eerst, afkorting tussen haakjes, dan pas in afgekorte vorm. Geldt voor vakjargon (CNC = Computer Numerical Control), afkortingen (BEP = Break-Even-Punt; CM = Contributiemarge), Franse/Engelse termen (reflet → spiegelpost), én vakbegrippen die in een vorig leerstuk staan ("voor wie [[<vorig-leerstuk>]] doorhad" is niet genoeg — geef één-zin-definitie + Meridia-getal). Een visualisatie (mermaid, beslisboom, vergelijkingsmatrix) gebruikt typisch al jargon — verplaats hem ALTIJD naar nà de uitleg van zijn termen, ook al staat hij in `antwoord_in_een_blik` van het script. Vuistregel: als de stagiair een term op een rij ziet die hij nog niet kent, is de term te vroeg.

12. **Pedagogische volgorde: introductie eerst, dan specifieke gevallen, dan uitzondering** — Niet beginnen met de uitzondering of het detail. Een sectie als "wat-is-X" moet eerst de hoofdregel neerzetten, dan de toepassing, dan de randgevallen. Bij twijfel: zet de uitzondering in een blockquote-aside (per regel 4b) of in de valkuilen-sectie aan het einde. Bij elke nieuwe term of concept dat je in lopende tekst neerschrijft: vraag jezelf "begrijpt de stagiair dit al?" Antwoord moet ja zijn op basis van wat er hierboven staat.
